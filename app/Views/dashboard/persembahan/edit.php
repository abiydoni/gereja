        ...
        <form action="<?= base_url('dashboard/persembahan/update/'.$persembahan['id_persembahan']) ?>" method="post">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="waktu_ibadah" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                        <option value="" <?= empty($persembahan['waktu_ibadah']) ? 'selected' : '' ?>>-- Semua / Tidak Berlaku --</option>
                        <option value="PAGI" <?= $persembahan['waktu_ibadah'] == 'PAGI' ? 'selected' : '' ?>>PAGI</option>
                        <option value="SIANG" <?= $persembahan['waktu_ibadah'] == 'SIANG' ? 'selected' : '' ?>>SIANG</option>
                        <option value="SORE" <?= $persembahan['waktu_ibadah'] == 'SORE' ? 'selected' : '' ?>>SORE</option>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="jumlah_pria" value="<?= $persembahan['jumlah_pria'] ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" placeholder="0">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Kehadiran Wanita</label>
                    <input type="number" name="jumlah_wanita" value="<?= $persembahan['jumlah_wanita'] ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" placeholder="0">
                </div>
            </div>
