...
<div class="max-w-full mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        <form action="<?= base_url('dashboard/renungan/update/'.$renungan['id_renungan']) ?>" method="post" enctype="multipart/form-data">
            
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Tanggal</label>
                    <input type="date" name="tanggal" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" required value="<?= $renungan['tanggal'] ?>">
                </div>
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                        <option value="aktif" <?= $renungan['status'] == 'aktif' ? 'selected' : '' ?>>Aktif</option>
                        <option value="tidak aktif" <?= $renungan['status'] == 'tidak aktif' ? 'selected' : '' ?>>Tidak Aktif</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <?php if(!empty($renungan['gambar'])): ?>
                    <div class="mb-3 relative w-full h-48 bg-slate-100 rounded-lg overflow-hidden group">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="judul" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" required value="<?= $renungan['judul'] ?>">
            </div>

            <div class="mb-6">
                <label class="block text-sm font-medium text-slate-700 mb-2">Isi Renungan</label>
                <textarea id="editor" name="isi" class="w-full px-4 py-2 border border-slate-300 rounded-lg"><?= $renungan['isi'] ?></textarea>
            </div>

            <div class="flex justify-end space-x-3">
                <a href="<?= base_url('dashboard/renungan') ?>" class="px-5 py-2.5 bg-slate-100 text-slate-600 font-medium rounded-lg hover:bg-slate-200 transition">Batal</a>
