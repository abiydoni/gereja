...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Render Session Config & Matrix Header
        function renderStructure() {
            // 1. Render Session Config List
            ...
            // 2. Render Matrix Header
            // Reset grid columns based on count (3 cols for tasks + configured sessions)
            // We need a dynamic grid-template-columns style because tailwind grid-cols-12 is fixed.
            // Or stick to grid but calculate col-span dynamically? 
            // Better: Use flex with fixed widths or percentage widths?
            // Or stick to grid-cols-12 but adjust spans.
            // If many sessions, grid-cols-12 breaks. 
            // Let's use CSS Grid with `grid-template-columns: 200px repeat(${sessions.length}, 1fr)` style on the container.
            ...
            sessions.forEach(sess => {
                ...
                div.textContent = `Petugas ${sess.name}`; // Just label, won't update live with input but that's fine for now or add listener
        ...
        // Listen for Session Name Changes to update Header Labels live (Optional polish)
        sessionConfigList.addEventListener('input', (e) => {
            if (e.target.name.includes('[name]')) {
                renderStructure(); // Re-render headers (a bit expensive but safe)
                // Actually re-rendering inputs loses focus. Better just update textContent by index.
                // For now, let's skip live update of header labels to avoid focus loss.
                // Just let renderStructure run on remove/add.
            }
        });
        ...
        addSessionBtn.addEventListener('click', () => {
            ...
            sessions.push({ id, name: `Sesi ${sessions.length + 1}`, time: '09:00', color });
            renderStructure();
        ...
        function createRow(roleValue = '', insertAfterNode = null) {
            ...
            // 2. Session Columns
            sessions.forEach(sess => {
                grid.appendChild(createCell(sess.id, '', sess.color));
            });
