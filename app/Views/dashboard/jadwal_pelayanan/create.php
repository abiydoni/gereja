            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4 px-2 bg-slate-50 p-3 rounded-xl border border-slate-200">
                ...
                <div class="col-span-3 flex flex-col gap-1">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="sess_pagi_active" id="check_pagi" value="1" checked class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500 session-toggle" data-target="pagi">
                        <label for="check_pagi" class="text-[10px] font-bold text-slate-600 uppercase">Sesi 1 (Pagi)</label>
                    </div>
                    <input type="time" name="sess_pagi_time" value="06:00" class="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:ring-blue-500 focus:border-blue-500 text-slate-600">
                </div>

                <!-- Sesi 2 Config -->
                <div class="col-span-3 flex flex-col gap-1">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="sess_siang_active" id="check_siang" value="1" checked class="w-4 h-4 text-orange-600 rounded border-slate-300 focus:ring-orange-500 session-toggle" data-target="siang">
                        <label for="check_siang" class="text-[10px] font-bold text-slate-600 uppercase">Sesi 2 (Siang)</label>
                    </div>
                    <input type="time" name="sess_siang_time" value="09:00" class="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:ring-orange-500 focus:border-orange-500 text-slate-600">
                </div>

                <!-- Sesi 3 Config -->
                <div class="col-span-3 flex flex-col gap-1">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="sess_sore_active" id="check_sore" value="1" checked class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 session-toggle" data-target="sore">
                        <label for="check_sore" class="text-[10px] font-bold text-slate-600 uppercase">Sesi 3 (Sore)</label>
                    </div>
                    <input type="time" name="sess_sore_time" value="17:00" class="w-full px-2 py-1 text-xs border border-slate-300 rounded focus:ring-indigo-500 focus:border-indigo-500 text-slate-600">
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // insertAfterNode: DOM Element to insert after. If null, append to end.
        function createRow(roleValue = '', insertAfterNode = null) {
            ...
            row.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                    ...
                    <div class="col-span-1 md:col-span-3">
                        ...
                        <textarea name="petugas_pagi[]" rows="2" class="input-pagi w-full px-2 py-1 bg-white border border-blue-100 rounded focus:ring-1 focus:ring-blue-100 outline-none transition-all text-xs min-h-[50px] placeholder-slate-300 resize-y" placeholder="-"></textarea>
                    </div>

                    <!-- Siang Input -->
                    <div class="col-span-1 md:col-span-3">
                        <label class="md:hidden block text-[10px] font-bold text-orange-400 uppercase tracking-wider mb-1">Siang</label>
                        <textarea name="petugas_siang[]" rows="2" class="input-siang w-full px-2 py-1 bg-white border border-orange-100 rounded focus:ring-1 focus:ring-orange-100 outline-none transition-all text-xs min-h-[50px] placeholder-slate-300 resize-y" placeholder="-"></textarea>
                    </div>
                    
                    <!-- Sore Input -->
                    <div class="col-span-1 md:col-span-3">
                        <label class="md:hidden block text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1">Sore</label>
                        <textarea name="petugas_sore[]" rows="2" class="input-sore w-full px-2 py-1 bg-white border border-indigo-100 rounded focus:ring-1 focus:ring-indigo-100 outline-none transition-all text-xs min-h-[50px] placeholder-slate-300 resize-y" placeholder="-"></textarea>
                    </div>
                </div>
            `;
            ...
            updateSessionVisibility(); // Re-apply visibility state to new row
        }

        // Initialize with default roles
        defaultRoles.forEach(role => createRow(role));

        // Use Event Delegation for Click Events on Container
        container.addEventListener('click', function(e) {
            ...
            if (removeBtn) {
                ...
                // Don't allow removing the last remaining row if needed, 
                // but usually fine to remove all.
                row.remove();
                
                // If all rows removed, maybe add one blank? 
