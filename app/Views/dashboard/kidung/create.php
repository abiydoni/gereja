...
<div class="mb-6 flex items-center space-x-3">
    ...
    <div>
        <h2 class="text-2xl font-bold text-slate-800 font-heading">Tambah Lagu</h2>
        <p class="text-slate-500 text-sm mt-1">Tambahkan kidung jemaat baru ke database.</p>
    </div>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden max-w-4xl">
    <div class="p-6 md:p-8">
        <form action="<?= base_url('dashboard/kidung/store') ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nomor Lagu <span class="text-red-500">*</span></label>
                    <input type="number" name="nomor" value="<?= old('nomor') ?>" required class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-accent focus:border-accent transition-all text-sm outline-none" placeholder="Contoh: 1">
                </div>
                
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Judul Lagu <span class="text-red-500">*</span></label>
                    <input type="text" name="judul" value="<?= old('judul') ?>" required class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-accent focus:border-accent transition-all text-sm outline-none" placeholder="Contoh: Haleluya! Pujilah">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nada Dasar</label>
                    <input type="text" name="nada_dasar" value="<?= old('nada_dasar') ?>" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-accent focus:border-accent transition-all text-sm outline-none" placeholder="Contoh: DO = F (4 Ketuk)">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Pengarang / Terjemahan</label>
                    <input type="text" name="pengarang" value="<?= old('pengarang') ?>" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-accent focus:border-accent transition-all text-sm outline-none" placeholder="M. Karosekali">
                </div>
            </div>
